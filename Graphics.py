...
def move_unit_by_mouse(pos, steps):
    ...
    Board.legal_move(Board.search_unit_by_place(pos),
                     steps)
...
def triangles_pressed(pos):  # checks if the mouse pressed on
    # 1 triangle and wait for another press so the turn can be
    if get_row_col(pos) is not None:
        if convert_pos_to_row(pos) >= 1:
            for pos2 in pygame.event.get():
                if pos2.type == pygame.MOUSEBUTTONDOWN:
                    if convert_pos_to_row(pos2) >= 1:
                        pos = convert_pos_to_row(pos)
                        pos2 = convert_pos_to_row(pos2)
                        move_unit_by_mouse(pos, (abs(pos - pos2) * Board.search_unit_by_place(pos2).color))
